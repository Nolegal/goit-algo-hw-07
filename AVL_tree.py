...
def find_max_value(node):
    ...
    return current.key
    #TODO Написати логіку повернення максимального значення
...
def find_min_value(node):
    ...
    return current.key
    #TODO Написати логіку повернення мінімального значення

def sum_of_values(node):
     if node is None:
         return 0
     return node.key+sum_of_values(node.left)+sum_of_values(node.right)
    #TODO Написати логіку обрахунку суми значень всіх вершин
...
root = AVLNode(0)
root.left = AVLNode(-2)
root.right = AVLNode(4)
root.left.left = AVLNode(-4)